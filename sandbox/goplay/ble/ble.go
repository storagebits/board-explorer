...
func onPeriphDiscovered(p gatt.Peripheral, a *gatt.Advertisement, rssi int) {
	if (a.LocalName == "BBC micro:bit [tavez]") {
		log.Printf("Preipheral Discovered: %s \n", p.Name())
		p.Device().StopScanning()
		p.Device().Connect(p);
	} else {
		log.Printf("NO UART from : %s with LocoalName %s\n", p.Name(), a.LocalName)
	}
}
		...
		if (service.UUID().Equal(uartServiceId)) {
			...
			for _, c := range cs {
					if (c.UUID().Equal(uartServiceRXCharId)) {
						...
						log.Printf("Writing %s\n", string([]byte{0x46,0x3A,0x00}))
						err := p.WriteCharacteristic(c, []byte{0x46,0x3A,0x00}, false)
						if err != nil {
							log.Fatalf("can't write : %s", err)
						}

						/*p.WriteCharacteristic(c, []byte{0x5C}, true)
						log.Printf("Wrote %s\n", string([]byte{0x5C}))
						p.WriteCharacteristic(c, []byte{0x72}, true)
						log.Printf("Wrote %s\n", string([]byte{0x72}))*/
					}
			}
		}
